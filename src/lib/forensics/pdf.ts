...
function fmtPdfDate(d?: string): string | undefined {
  ...
  const m =
    /^D?:?(\d{4})(\d{2})?(\d{2})?(\d{2})?(\d{2})?(\d{2})?/.exec(d);
...
export async function parsePdf(
  ...
  for (let i = 1; i <= total; i++) {
    ...
    const commonObjs: Record<string, any> =
      (page as any).commonObjs?._objs ?? {};
    ...
    for (const a of ann) {
      ...
      if (a.subtype === "Text" || a.subtype === "Popup") {
        annotations.push(`comment: ${a.contents?.slice(0, 80) ?? ""}`);
      }
    }
  ...
  const comments = annotations
    ...
    .map((c) => c.replace(/^comment:\s*/, ""));

  const encrypted =
    (doc as any)._pdfInfo?.encrypted ?? false;
